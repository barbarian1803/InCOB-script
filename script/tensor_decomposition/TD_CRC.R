#CRC
require(rTensor) 
x <- read.csv("../../data/cancer_data/cancer_expression_data/CRC/GSE104836_gene_exp.csv",sep="\t")
Z <- array(NA,c(dim(x)[1],10,2))
Z[,,1] <- data.matrix(x[,-1][,seq(1,20,by=2)])
Z[,,2] <- data.matrix(x[,-1][,seq(2,20,by=2)])
...
P <- pchisq(rowSums(scale(HOSVD$U[[1]][,1:3])^2),3,lower.tail=F)
